...
class EmbeddingOptimizationPolicy:
    ...
    def should_reembed(self, memory: MemoryItem) -> bool:
        ...
        if model_changed:
            ...
            if (
                memory.retrieval_stats
                and memory.retrieval_stats.retrieval_count >= self.min_retrieval_count
            ):
                return True
        ...
        if (
            memory.retrieval_stats
            and memory.retrieval_stats.performance_score < self.performance_threshold
        ):
            return True

        # Embedding too old?
        age_days = (
            datetime.now(timezone.utc) - ensure_utc(memory.embedding_metadata.last_updated)
        ).days
