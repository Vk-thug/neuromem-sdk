...
class MaintenanceWorker(BaseWorker):
    ...
    def _process_task(self, task):
        ...
        while retry_count < max_retries:
            try:
                ...
                self.metrics.record(
                    "task.duration", duration_ms, {"type": task.task_type.task_name}
                )
                ...
                self.metrics.increment(
                    "task.failed", {"type": task.task_type.task_name, "error": type(e).__name__}
                )
    ...
    def _proactive_maintenance(self):
        ...
        if self._should_consolidate():
            try:
                ...
                logger.error(
                    "Auto-consolidation failed", exc_info=True, extra={"error": str(e)[:200]}
                )
        ...
        if self._should_optimize():
            try:
                ...
                logger.error(
                    "Auto-optimization failed", exc_info=True, extra={"error": str(e)[:200]}
                )
    ...
    def _should_consolidate(self) -> bool:
        ...
        # Check memory count
        episodic_count = len(
            self.controller.episodic.get_all(limit=self.auto_consolidate_threshold + 1)
        )
    ...
    def _should_optimize(self) -> bool:
        ...
        time_since_last = (
            datetime.now(timezone.utc) - self.last_optimization
        ).total_seconds() / 3600
