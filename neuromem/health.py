...
def get_health_status(neuromem_instance) -> Dict[str, Any]:
    ...
    logger.info(
        ...
        extra={
            ...
            "unhealthy_checks": [
                k for k, v in health["checks"].items() if v["status"] != HealthStatus.HEALTHY
            ],
        },
    )
    ...
    try:
        ...
        if hasattr(backend, "_get_conn"):
            ...
            try:
                ...
                backend._put_conn(conn)
                return {"status": HealthStatus.UNHEALTHY, "type": "postgres", "error": str(e)[:200]}
        else:
        ...
        return {"status": HealthStatus.UNHEALTHY, "error": str(e)[:200]}
...
def _check_workers(controller) -> Dict[str, Any]:
    ...
    try:
        if not controller.async_enabled:
            return {"status": HealthStatus.HEALTHY, "message": "Async mode disabled"}
        ...
        if controller.ingest_worker:
            workers["ingest"] = {
                ...
                "thread_alive": (
                    controller.ingest_worker._thread.is_alive()
                    if controller.ingest_worker._thread
                    else False
                ),
            }
        ...
        if controller.maintenance_worker:
            workers["maintenance"] = {
                ...
                "thread_alive": (
                    controller.maintenance_worker._thread.is_alive()
                    if controller.maintenance_worker._thread
                    else False
                ),
            }

        # Determine status
        all_healthy = all(w["running"] and w["thread_alive"] for w in workers.values())
...
def _check_queues(controller) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"status": status, "depths": queue_depths, "message": message}
...
def _check_external_apis() -> Dict[str, Any]:
    ...
    try:
        ...
        return {"status": HealthStatus.DEGRADED, "error": str(e)[:200]}
...
def _check_dead_letter_queue(controller) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"status": status, "size": dlq_size, "message": message}
