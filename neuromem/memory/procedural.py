...
class ProceduralMemory:
    ...
    def get_all(self, limit: int = 100) -> List[MemoryItem]:
        ...
        return self.backend.list_all(
            self.user_id, memory_type=MemoryType.PROCEDURAL.value, limit=limit
        )
    ...
    def extract_style_profile(self, messages: List[str]) -> Dict[str, Any]:
        ...
        tech_density = sum(
            sum(1 for term in tech_terms if term in m.lower()) for m in messages
        ) / len(messages)

        # Detect tone
        analytical_indicators = ["why", "how", "explain", "because", "therefore"]
        analytical_score = sum(
            sum(1 for ind in analytical_indicators if ind in m.lower()) for m in messages
        ) / len(messages)
        ...
        return {
            ...
            "length_category": (
                "concise" if avg_length < 20 else "detailed" if avg_length > 50 else "moderate"
            ),
            "prefers_bullets": prefers_bullets > 0.3,
            "technical_depth": (
                "high" if tech_density > 2 else "medium" if tech_density > 0.5 else "low"
            ),
