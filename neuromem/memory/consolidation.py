...
class ConsolidationEngine:
    ...
    def extract_facts(self, memories: List[MemoryItem]) -> List[Dict[str, Any]]:
        ...
        conversation_text = "\n\n".join([f"[{mem.created_at}] {mem.content}" for mem in memories])
...
Return ONLY the JSON array, no other text."""
        ...
        try:
            facts_json = chat_completion(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a memory consolidation system that extracts semantic facts from conversations.",
                    },
    ...
    def summarize_conversation(
        self, memories: List[MemoryItem], max_summary_length: int = 200
...
{{
  ...
  "salience": 0.0-1.0
        ...
        try:
            summary_json = chat_completion(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a memory consolidation system that creates concise summaries.",
                    },
                    {"role": "user", "content": prompt},
                ],
                temperature=0.3,
    ...
    def merge_similar_facts(
        self, facts: List[Dict[str, Any]], similarity_threshold: float = 0.85
    ) -> List[Dict[str, Any]]:
        """
    ...
    def apply_decay(self, memories: List[MemoryItem], decay_threshold: float = 0.3) -> List[str]:
        """
